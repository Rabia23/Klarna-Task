...
class MathAPIViewTestCase(TestCase):
    ...
    @ddt.unpack
    def test_url_calculates_function_result(self, query_params, response_data):
        """
        Test that url calculates the function results according to given
    ...
    @ddt.unpack
    def test_url_saves_function_log_in_db(
        self, func_name, query_params, input_values
    ):
        """Test that url saves the function logs in the database."""
    ...
    @ddt.data(
        ...
        (
            {"function": "fib", "n": 3},
            "Not a valid method. Options are ('fibonacci', 'ackermann', 'factorial')",  # noqa:E501
        ),
        (
    ...
    @ddt.unpack
    def test_url_with_invalid_params_returns_error(
...
class FunctionLogsListAPIViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.func_logs_url = reverse("func-logs")
    ...
    def test_url_returns_paginated_data(self):
        """Test that url returns the paginated data in the response."""
        res = self.client.get(self.func_logs_url).json()
        self.assertEqual(res.get("count"), len(self.query_params_list))
        self.assertEqual(
            res.get("next"), "http://testserver/api/func-logs/?page=2"
        )
    ...
    @ddt.unpack
    def test_url_with_filters_returns_filtered_data(
        self, query_params, expected_count
    ):
        """Test that url returns the filtered data in the response."""
    ...
    def test_url_with_invalid_filter_param_returns_error_response(
        self, query_params
    ):
        """
        Test that url with the invalid filter params returns the error
        in the response.
        """
        res = self.client.get(self.func_logs_url, query_params).json()
        self.assertEqual(
